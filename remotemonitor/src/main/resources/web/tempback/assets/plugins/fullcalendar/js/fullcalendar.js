...
function Calendar(element, instanceOptions) {
	...
	function getLocaleData(langCode) {
		...
		return f.call(moment, langCode) ||
			f.call(moment, 'en'); // the newer localData could return null, so fall tempback to en
	}
...
// TODO: back liquid width, especially in IE6
		...
		eventElement.resizable({
			...
			stop: function(ev, ui) {
				...
				if (snapDelta) {
					...
					showEvents(event, eventElement);
					// BUG: if event was really short, need to put title tempback in span
				}
			}
		});
	}
...
function SelectionManager() {
	...
	// TODO: better date normalization. see notes in automated back
